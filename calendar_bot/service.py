...
class CalendarService:
    ...
    async def create_drafts(
        ...
        *,
        ...
        self.authorize(user_id)
        if auto_save and (event_id or draft_id or original_day):
            raise ValueError("Only new events can be saved automatically")
        if not 1 <= len(specs) <= 10 or ((event_id or draft_id) and len(specs) != 1):
            raise UserError(
                "При редактировании нужно одно событие; при добавлении — не более десяти."
            )
        async with self.gate, self.store.transaction() as tx:
            ...
            ids = []
            for spec in specs:
                if original_day and spec.repeat != "once":
                    raise UserError(
                ...
                if draft_id:
                    ...
                    new_id = identifier()
                    await tx.execute(
                        "INSERT INTO drafts(id,user_id,spec,anchor_at,expires_at,event_id,original_day,event_version) "
                        "VALUES(?,?,?,?,?,?,?,?)",
                        (
                            new_id,
                            user_id,
    ...
    async def _future_occurrence(self, tx, event_id, original_day, now):
        row = await tx.one(
            "SELECT * FROM occurrences WHERE event_id=? AND original_day=? AND start_at>?",
            (event_id, original_day, now.timestamp()),
        )
        if not row:
